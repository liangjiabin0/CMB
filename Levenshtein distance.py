...
class MolecularGenerator(nn.Module):
    def __init__(self, config_json_path, tokenizer_path):
        ...
        self.attn_layer_norm = nn.LayerNorm(768)
    ...
    def ir_forward(self, ir):
        ...
        x = self.attn_layer_norm(attn_output + x)
    ...
    def infer(self, num_beams=10, num_return_sequences=None, max_length=512, **kwargs):
        ir_embedding = self.ir_forward(kwargs.pop("ir"))  # [batch_size, 50, 768]
        result = self.model.generate(
            max_length=max_length,
            num_beams=num_beams,
            num_return_sequences=num_return_sequences or num_beams,
            inputs_embeds=ir_embedding,
            decoder_start_token_id=self.model.config.decoder_start_token_id or self.model.config.pad_token_id,
        )
        smiles = [
...
def main(
    ...
    for epoch in range(num_train_epochs):
        model.train()
        epoch_loss = 0
        progress_bar = tqdm(train_dataloader, desc=f"Epoch {epoch + 1}/{num_train_epochs}", total=len(train_dataloader))
        for step, batch in enumerate(progress_bar):
            ...
            progress_bar.set_postfix({'loss': f'{epoch_loss / (step + 1):.4f}', 'lr': f'{optimizer.param_groups[0]["lr"]:.2e}'})
        torch.save(
            model.state_dict(),
...
def run_test(test_data_path, weights_path, batch_size, output_csv):
    ...
    run_test(
        ...
        weights_path="/root/Desktop/我的网盘/liangjiabin/outputs/checkpoint-epoch-2混合精度应用训练.pt",
        batch_size=16,
        output_csv="results混合精度.csv"
    )   
...
def evaluate_model(test_data_path, weights_path, batch_size):
    ...
    test_dataloader = DataLoader(
        IRDataset(db_path=test_data_path),
        shuffle=False,
        collate_fn=MyCollator(tokenizer=model.tokenizer, max_length=512),
        batch_size=batch_size,
        drop_last=False,
    ...
    total_count = 0
    with torch.no_grad():
        for idx, batch in tqdm(enumerate(test_dataloader), desc="Testing", total=len(test_dataloader)):
            total_count += 1
            predictions = model.infer(
                num_beams=10,
                num_return_sequences=10,
            ...
            ground_truth = ground_truth_smiles[idx]
            
            # 计算第一个候选的莱文斯坦距离
            distance = levenshtein_distance(ground_truth, predictions[0])
    ...
    avg_levenshtein_distance = total_levenshtein_distance / total_count
    print(f"Average Levenshtein Distance (Top 1): {avg_levenshtein_distance:.4f}")
...
weights_path = os.path.join(current_dir, "/root/Desktop/我的网盘/liangjiabin/outputs/checkpoint-epoch-2混合精度应用训练.pt") #测试时要把模型路径换成对应的
batch_size = 1
